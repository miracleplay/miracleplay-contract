...
contract MiracleStoreEscrow is PermissionsEnumerable, Multicall, ContractMetadata {
    ...
    function setItem(uint256 itemId, address tokenAddress, uint256 price, string memory name) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function removeItem(uint256 itemId, address tokenAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateItemPrice(uint256 itemId, address tokenAddress, uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(items[itemId][tokenAddress].exists, "Item does not exist");
