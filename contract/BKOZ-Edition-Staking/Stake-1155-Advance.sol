...
contract ERC1155Staking is ReentrancyGuard, PermissionsEnumerable, ERC1155Holder, ContractMetadata, Multicall{
    ...
    // External function to calculate the reward for a given user, dao.
    function calculateRewards(address _user) external view returns (uint256 userReward, uint256 daoFee) {
        ...
        userReward = reward - daoFee;

        return (userReward, daoFee);
    }
    ...
    function _claimReward(address _user, bool isAdmin) internal {
        ...
        // Transfer the DAO fee to the DAO address if applicable.
        if (daoFee > 0) {
            rewardsToken.transfer(daoAddress, daoFee);
        }
        ...
        // Calculate the user's net reward after deducting fees.
        uint256 userReward = reward - daoFee - adminFee;
    ...
    function getRemainingStakingRewards() public view returns (uint256) {
        return MAX_REWARD - rewardsToken.balanceOf(address(this));
    }
